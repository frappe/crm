...
def create_prospect_against_crm_deal():
    ...
    try:
        ...
        frappe.log_error(
            ...
            f"Error while creating prospect against Opportunity: {frappe.form_dict.get('opportunity_id')}",
        )
        pass

    create_contacts(
        json.loads(doc.contacts), prospect.company_name, "Prospect", prospect_name
    )
...
def create_contacts(contacts, organization=None, link_doctype=None, link_docname=None):
    for c in contacts:
        ...
        if existing_contact:
            ...
            if c.get("email"):
                contact.append(
                    "email_ids", {"email_id": c.get("email"), "is_primary": 1}
                )

            if c.get("mobile_no"):
                contact.append(
                    "phone_nos",
                    {"phone": c.get("mobile_no"), "is_primary_mobile_no": 1},
                )
...
def create_address(doctype, docname, address):
    ...
    try:
        ...
        frappe.log_error(
            frappe.get_traceback(), f"Error while creating address for {docname}"
        )
...
def link_doc(doc, link_doctype, link_docname):
    already_linked = any(
        [
            (link.link_doctype == link_doctype and link.link_name == link_docname)
    ...
    if not already_linked:
        doc.append(
            "links",
            {
                ...
                "link_name": link_docname,
                "link_title": link_docname,
            },
        )
