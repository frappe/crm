...
class Twilio:
    ...
    def get_update_call_status_callback_url(self):
        url_path = (
            "/api/method/next_crm.integrations.twilio.api.update_call_status_info"
        )
        return get_public_url(url_path)
...
def get_the_call_attender(owners, caller=None):
    ...
    if len(current_loggedin_users) > 1 and caller:
        opportunity_owner = frappe.db.get_value(
            "Opportunity", {"mobile_no": caller}, "opportunity_owner"
        )
        if not opportunity_owner:
            opportunity_owner = frappe.db.get_value(
                "Lead", {"mobile_no": caller, "converted": False}, "lead_owner"
            )
