...
def generate_access_token():
    ...
    from_number = frappe.db.get_value(
        "Twilio Agents", frappe.session.user, "twilio_number"
    )
    if not from_number:
        return {
            "ok": False,
...
def create_call_log(call_details: TwilioCallDetails):
    call_log = frappe.get_doc(
        {**call_details.to_dict(), "doctype": "CRM Call Log", "medium": "Twilio"}
    )
    call_log.reference_docname, call_log.reference_doctype = (
        get_lead_or_opportunity_from_number(call_log)
    )
...
def add_note_to_call_log(call_sid, note):
    ...
    sid = (
        ...
        else call_details.parent_call_sid
    )
