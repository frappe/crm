...
def update_call_log(call_sid, status=None):
	...
	if call_log.note and call_log.reference_docname:
		frappe.db.set_value("NCRM Note", call_log.note, "reference_doctype", call_log.reference_doctype)
		frappe.db.set_value("NCRM Note", call_log.note, "reference_docname", call_log.reference_docname)
	call_log.flags.ignore_permissions = True
	call_log.save()
	frappe.db.commit()
