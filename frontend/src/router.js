...
const routes = [
  ...
  {
    alias: '/lead-segments',
    path: '/lead-segments/view/:viewType?',
    name: 'Lead Segments',
    component: () => import('@/pages/LeadSegments.vue'),
  },
  {
    path: '/lead-segments/:segmentId',
    name: 'LeadSegment',
    component: () => import('@/pages/LeadSegment.vue'),
    props: true,
  },
  {
  ...
  if (isLoggedIn && to.name !== 'Not Permitted' && !isCrmUser()) {
    ...
    if (!viewType) {
      const doctypeMap = {
        ...
        'Lead Segments': 'CRM Lead Segment',
        Notes: 'FCRM Note',
        Tasks: 'CRM Task',
