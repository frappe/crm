<template>
  <!-- Desktop sidebar: always visible -->
  <!-- Mobile: fixed overlay, toggled by isOpen -->
  <aside
    :class="[
      'fc-sidebar flex flex-col bg-gray-900 text-gray-100 transition-transform duration-200 flex-shrink-0',
      isMobile ? 'fixed top-0 left-0 z-50 h-full shadow-2xl' : 'relative h-full',
      isMobile && !isOpen ? '-translate-x-full' : 'translate-x-0',
    ]"
    style="width: 220px;"
  >
    <div class="relative z-50 flex flex-col h-full bg-gray-900" style="width: 220px;">
      <div class="px-4 py-3 border-b border-gray-700 font-bold text-sm tracking-wide text-gray-400 uppercase">
        Finance
      </div>
      <nav class="flex-1 overflow-y-auto py-2">
        <a
          v-for="section in visibleSections"
          :key="section.key"
          :href="section.hash"
          :class="[
            'flex items-center gap-3 px-4 py-2 text-sm cursor-pointer hover:bg-gray-800 transition-colors',
            activeSection === section.key
              ? 'border-l-4 border-blue-500 bg-gray-700 text-white'
              : 'border-l-4 border-transparent text-gray-300',
          ]"
          @click.prevent="navigate(section)"
        >
          <!-- Icon via inline svg snippets mapped from icon name -->
          <span class="w-5 h-5 flex-shrink-0" v-html="getIconSvg(section.icon)" />
          <span>{{ section.label }}</span>
          <span
            v-if="section.badge && badgeCount > 0"
            class="ml-auto text-xs bg-red-500 text-white rounded-full px-1.5 py-0.5 min-w-[18px] text-center"
          >{{ badgeCount }}</span>
        </a>
      </nav>
    </div>
  </aside>
</template>

<script setup>
import { computed } from 'vue'
import { SIDEBAR_SECTIONS } from '../constants/sidebarConfig.js'
import { useSidebar } from '../composables/useSidebar.js'
import { useBreakpoint } from '../composables/useBreakpoint.js'

const props = defineProps({
  activeSection: { type: String, default: 'dashboard' },
  badgeCount: { type: Number, default: 0 },
  userRoles: { type: Array, default: () => [] },
})
const emit = defineEmits(['navigate'])

const { isOpen, close } = useSidebar()
const { isMobile } = useBreakpoint()

const visibleSections = computed(() => {
  const roles = props.userRoles
  const isAdmin = roles.includes('System Manager') || window.frappe?.session?.user === 'Administrator'
  const isFM = isAdmin || roles.includes('Finance Manager')
  const isAR = isFM || roles.includes('Accounts User') || roles.includes('Accounts Manager') || roles.includes('AR Accountant')
  const isAP = isFM || roles.includes('Accounts User') || roles.includes('Accounts Manager') || roles.includes('AP Accountant')

  return SIDEBAR_SECTIONS.filter(s => {
    if (s.visibleRoles === 'fm')  return isFM
    if (s.visibleRoles === 'ar')  return isAR
    if (s.visibleRoles === 'ap')  return isAP
    if (s.visibleRoles === 'all') return isFM || isAR || isAP
    return true
  })
})

function navigate(section) {
  if (isMobile.value) close()
  emit('navigate', section.key)
}

// Minimal inline SVG map — exact Lucide icon paths for the 12 sidebar icons
const ICON_SVGS = {
  'layout-dashboard': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="7" height="9" x="3" y="3" rx="1"/><rect width="7" height="5" x="14" y="3" rx="1"/><rect width="7" height="9" x="14" y="12" rx="1"/><rect width="7" height="5" x="3" y="16" rx="1"/></svg>',
  'inbox': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg>',
  'receipt': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1Z"/><path d="M16 8h-6a2 2 0 1 0 0 4h4a2 2 0 1 1 0 4H8"/><path d="M12 17.5v-11"/></svg>',
  'credit-card': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" x2="22" y1="10" y2="10"/></svg>',
  'wallet': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12V7H5a2 2 0 0 1 0-4h14v4"/><path d="M3 5v14a2 2 0 0 0 2 2h16v-5"/><path d="M18 12a2 2 0 0 0 0 4h4v-4Z"/></svg>',
  'building-2': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18Z"/><path d="M6 12H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2"/><path d="M18 9h2a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-2"/><path d="M10 6h4"/><path d="M10 10h4"/><path d="M10 14h4"/><path d="M10 18h4"/></svg>',
  'landmark': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="3" x2="21" y1="22" y2="22"/><line x1="6" x2="6" y1="18" y2="11"/><line x1="10" x2="10" y1="18" y2="11"/><line x1="14" x2="14" y1="18" y2="11"/><line x1="18" x2="18" y1="18" y2="11"/><polygon points="12 2 20 7 4 7"/></svg>',
  'scale': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m16 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="m2 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"/></svg>',
  'book-open': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>',
  'bar-chart-2': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" x2="18" y1="20" y2="10"/><line x1="12" x2="12" y1="20" y2="4"/><line x1="6" x2="6" y1="20" y2="14"/></svg>',
  'handshake': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m11 17 2 2a1 1 0 1 0 3-3"/><path d="m14 14 2.5 2.5a1 1 0 1 0 3-3l-3.88-3.88a3 3 0 0 0-4.24 0l-.88.88a1 1 0 1 1-3-3l2.81-2.81a5.79 5.79 0 0 1 7.06-.87l.47.28a2 2 0 0 0 1.42.25L21 4"/><path d="m21 3 1 11h-2"/><path d="M3 3 2 14l6.5 6.5a1 1 0 1 0 3-3"/><path d="M3 4h8"/></svg>',
  'settings': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg>',
}

function getIconSvg(name) {
  return ICON_SVGS[name] || ICON_SVGS['settings']
}
</script>

