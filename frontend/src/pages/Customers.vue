<template>
  <LayoutHeader>
    ...
    <template #right-header>
      <CustomActions v-if="customersListView?.customListActions" :actions="customersListView.customListActions" />
      <Button variant="solid" :label="__('Create')" @click="showCustomerModal = true">
        <template #prefix>
  ...
  <ViewControls ref="viewControls" v-model="customers" v-model:loadMore="loadMore" v-model:resizeColumn="triggerResize"
    v-model:updatedPageCount="updatedPageCount" doctype="Customer" />
  <CustomersListView ref="customersListView" v-if="customers.data && rows.length"
    v-model="customers.data.page_length_count" v-model:list="customers" :rows="rows" :columns="customers.data.columns"
    :options="{
      showTooltip: false,
      resizeColumn: true,
  ...
  <div v-else-if="customers.data" class="flex h-full items-center justify-center">
  ...
  <QuickEntryModal v-if="showQuickEntryModal" v-model="showQuickEntryModal" doctype="Customer" />
...
const rows = computed(() => {
  ...
  return customers.value?.data.data.map((customer) => {
    ...
    customers.value?.data.rows.forEach((row) => {
      ...
      let fieldType = customers.value?.data.columns?.find(
      ...
      if (fieldType && fieldType == 'Currency') {
        _rows[row] = getFormattedCurrency(row, customer)
      }

      if (fieldType && fieldType == 'Float') {
        _rows[row] = getFormattedFloat(row, customer)
      }

      if (fieldType && fieldType == 'Percent') {
        _rows[row] = getFormattedPercent(row, customer)
      }
