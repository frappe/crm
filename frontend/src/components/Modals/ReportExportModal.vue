...
import { triggerDownload } from '@/utils/reports'
...
async function handleExport() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // If server returned a JSON error wrapped as 200
      if (blob.type.includes('application/json')) {
        const text = await blob.text()
        const parsed = JSON.parse(text)
        if (parsed.exc) {
          throw new Error(parsed.exc)
        }
      }
    ...
    throw new Error(__('Server export request failed'))
  } catch (err: any) {
    toast.error(err?.message || __('Failed to export report'))
  } finally {
