...
function uploadFile(file) {
  avito.value.attach = file.file_url
  avito.value.content_type = fileType.value
  sendAvitoMessage()
  capture('avito_upload_file')
}
...
async function sendAvitoMessage() {
  let args = {
    ...
    chat_id: doc.value.data.avito_chat_id,
    attach: avito.value.attach || '',
    reply_to: reply.value?.name || '',
    content_type: avito.value.content_type,
  }
  content.value = ''
  fileType.value = ''
  avito.value.attach = ''
  avito.value.content_type = 'text'
  reply.value = {}
  createResource({
    url: 'crm.api.avito.create_avito_message',
    params: args,
    auto: true,
  })
}
