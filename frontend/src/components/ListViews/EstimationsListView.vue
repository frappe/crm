<template>
  <ListView :class="$attrs.class" :columns="columns" :rows="rows" :options="{
    ...
    <ListRows v-slot="{ idx, column, item, row }" :rows="rows" doctype="CRM Estimation">
      <ListRowItem :item="item" :align="column.align" class="overflow-hidden">
        ...
        <template #default="{ label }">
          <div v-if="['modified', 'creation'].includes(column.key)" class="truncate text-base" @click="
            (event) => emit('applyFilter', { event, idx, column, item, firstColumn: columns[0] })
          ">
  ...
  <ListBulkActions ref="listBulkActionsRef" v-model="list" doctype="CRM Estimation" />
</template>
