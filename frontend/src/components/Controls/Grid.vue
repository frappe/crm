<template>
    <div class="flex flex-col text-base">
      <div v-if="label" class="mb-1.5 text-sm text-ink-gray-5">{{ label }}</div>
  
      <div class="rounded border border-gray-100">
        <!-- Header -->
        <div
          class="grid items-center rounded-t-sm bg-surface-gray-2 text-ink-gray-5"
        ...
        <template v-if="rows.length">
          <Draggable class="w-full" v-model="rows" group="rows" item-key="name">
            <template #item="{ element: row, index }">
              <div
                class="grid-row grid cursor-pointer items-center border-b border-gray-100 bg-surface-white last:rounded-b last:border-b-0"
                :style="{ gridTemplateColumns: gridTemplateColumns }"
              >
                <div
                ...
                <div
                  class="flex h-full items-center justify-center border-r p-2 text-sm text-ink-gray-8"
                >
                  {{ index + 1 }}
                </div>
                <div
