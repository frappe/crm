...
<script setup>
// Avaya call UI — mirrors ExotelCallUI.vue (E4-S3). It registers the provider surface so
// the CallUI dialog can offer "Avaya" once CRM Avaya Settings is enabled.
//
// NOTE (E5/E6 gated): the outbound handler `crm.integrations.avaya.handler.make_a_call`
// and the real `avaya_call` webhook payload shape are DISCOVERY items pending Tiberbu's
// Avaya edition + credentials. `updateStatus()` below is a DEFENSIVE, generic mapper over
// common CTI fields — it must be finalized against the confirmed Avaya contract in E5.
// Until the handler exists, clicking call surfaces a graceful error toast (not a crash).
...
// call_sid resolves from whichever id the Avaya payload / make_a_call return carries.
function callSid() {
  return (
    callData.value?.CallSid ||
    callData.value?.call_id ||
...
function makeOutgoingCall(number) {
  ...
  createResource({
    url: 'crm.integrations.avaya.handler.make_a_call',
  ...
  $socket.on('avaya_call', (data) => {
    ...
    if (!showCallPopup.value && !showSmallCallPopup.value) {
      ...
      if (agent && agent == (user || user.value)) {
        ...
        // Outgoing call
        phoneNumber.value = data.To || data.to
      }
    }
  })
}
...
/**
 * DEFENSIVE placeholder status mapper (E4-S3). Real Avaya webhook payloads are a
 * discovery item (E5/E6) — this maps common, provider-agnostic CTI fields so the UI
 * behaves reasonably in the interim without crashing on an unknown shape. Finalize
 * against Tiberbu's confirmed Avaya contract in E5.
 */
function updateStatus(data) {
  const status = (data.status || data.Status || data.state || '').toLowerCase()
  const event = (data.event || data.EventType || '').toLowerCase()

  if (status === 'ringing' || event === 'ringing') return 'Ringing...'
  if (
    status === 'answered' ||
    status === 'in-progress' ||
    status === 'connected' ||
  ...
  if (status === 'no-answer' || status === 'missed' || status === 'busy') {
  ...
  if (
    ...
    counterUp.value?.stop()
    const secs = parseInt(data.duration || data.CallDuration || 0)
    if (!Number.isNaN(secs)) callDuration.value = counterUp.value?.getTime(secs)
    return 'Call ended'
  }
  return 'Calling...'
}
