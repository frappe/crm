        ...
        >
          ...
          <div class="flex items-center gap-2 shrink-0">
            <Button
              ...
              :loading="removingReport === report.report"
...
const pinnedReportNames = computed(() => {
  ...
  if (pinnedReports.data) {
    for (const r of pinnedReports.data) set.add(r.report)
  }
  return set
})
...
async function saveReports() {
  ...
  try {
    ...
    if (added.length) {
      toast.success(__('{0} report(s) added', [added.length]))
    }
    // tell the user which ones weren't added, and why
    for (const { report, reason } of skipped) {
      toast.error(__("Couldn't add {0}: {1}", [report, reason]))
    }
    showAddModal.value = false
    selectedReportNames.value = []
    searchQuery.value = ''
...
async function removeReport(report: { report: string }) {
  const reportKey = report.report
  if (!reportKey) return
