...
class CRMDeal(Document):
    ...
    if TYPE_CHECKING:
        ...
        sla_status: DF.Literal[
            "", "First Response Due", "Rolling Response Due", "Failed", "Fulfilled"
        ]
        source: DF.Link | None
        status: DF.Link
    ...
    def validate(self):
        ...
        if (
            not self.is_new()
            and self.has_value_changed("deal_owner")
            and self.deal_owner
        ):
        ...
        if self.is_new() and not self.status:
            if frappe.db.exists("CRM Deal Status", "Qualification"):
                ...
                self.status = frappe.get_all(
                    "CRM Deal Status", {"type": "Open"}, pluck="name"
                )[0]
    ...
    def assign_agent(self, agent):
        ...
        assign(
            {"assign_to": [agent], "doctype": "CRM Deal", "name": self.name},
    ...
    def update_default_probability(self):
        ...
        if not self.probability or self.probability == 0:
            self.probability = (
                frappe.db.get_value("CRM Deal Status", self.status, "probability") or 0
            )
    ...
    def update_expected_deal_value(self):
        ...
        if (
            frappe.db.get_single_value(
                "FCRM Settings", "auto_update_expected_deal_value"
            )
    ...
    def validate_forecasting_fields(self):
        ...
        if frappe.db.get_single_value("FCRM Settings", "enable_forecasting"):
            if not self.expected_deal_value or self.expected_deal_value == 0:
                frappe.throw(
                    _("Expected deal value is required."), frappe.MandatoryError
                )
    ...
    def validate_lost_reason(self):
        ...
        if (
            self.status
            and frappe.get_cached_value("CRM Deal Status", self.status, "type")
            == "Lost"
        ):
            if not self.lost_reason:
                frappe.throw(
                    _("Please specify a reason for losing the deal."),
                    frappe.ValidationError,
                )
            elif self.lost_reason == "Other" and not self.lost_notes:
                frappe.throw(
                    _("Please specify the reason for losing the deal."),
                    frappe.ValidationError,
                )
        if self.has_value_changed("status"):
...
def remove_contact(deal: str, contact: str):
    if not frappe.has_permission("CRM Deal", "write", deal):
        frappe.throw(
            _("Not allowed to remove contact from Deal"), frappe.PermissionError
        )
...
def set_primary_contact(deal: str, contact: str):
    if not frappe.has_permission("CRM Deal", "write", deal):
        frappe.throw(
            _("Not allowed to set primary contact for Deal"), frappe.PermissionError
        )
