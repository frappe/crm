...
def get_base_activities(name, doctype, avoid_fields=None):
	get_docinfo("", doctype, name)
	docinfo = frappe.response["docinfo"]
	doc_meta = frappe.get_meta(doctype)
	fields = {
		field.fieldname: {"label": field.label, "options": field.options} for field in doc_meta.fields
	}
	...
	return doc, activities, calls, notes, tasks, attachments, docinfo
...
def get_deal_activities(name):
	...
	doc, activities, calls, notes, tasks, attachments, docinfo = get_base_activities(
		name, "CRM Deal", avoid_fields
	)
	...
	if lead:
		lead_activities, lead_calls, lead_notes, lead_tasks, lead_attachments = get_lead_activities(lead)
		activities.extend(lead_activities)
		calls.extend(lead_calls)
		notes.extend(lead_notes)
		tasks.extend(lead_tasks)
		attachments.extend(lead_attachments)
	...
	# Set is_lead flag for all activities
	for activity in activities:
		activity["is_lead"] = False
...
def get_contact_activities(name):
	avoid_fields = [
		"response_by",
		"sla_creation",
		"sla",
