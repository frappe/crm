...
def handle_incoming_call() -> None:
	...
	details = IncomingCallDetails(caller=caller, callee=callee, channel_id=channel_id, call_id=call_id)
...
def make_http_request(
	...
	try:
		...
		frappe.log_error(
			title=f"Error while making request of type {request_type.upper()} to Yeastar API: {e!s}",
			message=frappe.get_traceback(),
		)
